...
const mapStateToProps = (state, ownProps) => { 
  return {
    ...
    noOfUnseenNotification: state.notifications.notifications ? values(
        state.notifications.notifications)
      .filter(p => Boolean(p.seen) === false).length : 0,
...
const AppWrapperHOC = Page => class AppWrapper extends React.Component {
  ...
  componentDidMount() {
    ...
    this.props.getNotifications("wulf@semada.io")
